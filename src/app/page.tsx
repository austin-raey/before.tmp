...
export default function Page() {
	return (
		<div className="bg-linear-to-br from-black to-75% to-neutral-900 text-neutral-400">
			...
			<div className="relative z-10 mx-auto min-h-svh max-w-2xl space-y-4 overflow-hidden px-6 py-20">
				<header className="space-y-2">
					<h1 className="font-serif text-6xl tracking-tighter sm:text-8xl">
						...
						<span className="font-extralight text-neutral-400">
							.
							<span className="underline decoration-2 decoration-wavy underline-offset-8 sm:decoration-[0.25rem]">
								tmp
							</span>
						</span>
					</h1>
					<p className="text-xl">
						A <span className="font-semibold text-white">Next.JS</span> starter
						template with a focus on performance, developer experience, and
						minimalism.
					</p>
				</header>
				...
				<nav className="flex flex-col gap-3 sm:flex-row">
					...
					<details className="group text-sm">
						<summary className="cursor-pointer hover:text-white [&::marker]:content-none">
							CLI{" "}
							<i className="icon-[lucide--chevron-down] group-open:icon-[lucide--chevron-up] size-4 align-middle" />
						</summary>
						<div className="text-white">Coming Soon!</div>
						<div className="flex flex-col gap-2">
							{[{ tool: "npm" }, { tool: "pnpm" }, { tool: "bun" }].map(
								({ tool }) => (
									<button
										...
										onClick={() => {
											navigator.clipboard.writeText(
												`${tool} create tbd@latest my-next-app`,
											);
										}}
										type="button"
									>
										<i className="icon-[lucide--copy] mr-1 align-text-bottom" />
										<code>
											<span className="font-semibold">{tool}</span> create
											tbd@latest my-next-app
										</code>
									</button>
								),
							)}
				...
				<section className="-mx-4 grid gap-4 sm:grid-cols-2">
					{[
						{
							description:
								"Optimized for speed with Bun runtime and minimal dependencies.",
							icon: "icon-[lucide--feather]",
							title: "Performance",
						},
						{
							description:
								"Includes ESLint, Prettier, and VSCode settings for a smooth development workflow.",
							icon: "icon-[lucide--keyboard]",
							title: "Developer Experience",
						},
						{
							description:
								"Opinionated project structure to help you get started quickly and stay organized.",
							icon: "icon-[lucide--layout]",
							title: "Structure",
						},
						{
							description:
								"Hand-picked and familiar packages to ensure easy integration and maintainability.",
						...
						>
							<i
								className={`${feature.icon} absolute inset-y-2 right-0 m-3 size-[8ch] opacity-10`}
							/>
							<h2 className="font-semibold font-serif text-2xl text-neutral-200">
								{feature.title}
							</h2>
